...
* **`restartPolicy`**: Autoscaler V1 and V2 both require `restartPolicy: Never` on all worker pods. However, KubeRay automatically sets `restartPolicy: Never` on worker pods if autoscaling V2 is enabled or the worker group's `restartPolicy` is unset with autoscaler V1. If you explicitly set `restartPolicy` to a value other than `Never` on a worker group while using autoscaler V1, KubeRay emits a warning log and a Kubernetes event.
